...
class AttnProcessor2_0:
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("AttnProcessor2_0 requires PyTorch 2.0+. Please upgrade PyTorch.")
    ...
    def __call__(
        ...
        del attention_mask, encoder_hidden_states
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        del cross_attn_norm, added_kv_proj_dim
    ...
    def forward(self, hidden_states: torch.Tensor, temb: torch.Tensor, rotary_emb: torch.Tensor) -> torch.Tensor:
...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.condition_embedder = TimeEmbedding(dim=inner_dim, time_freq_dim=freq_dim, time_proj_dim=inner_dim * 6)
